...
var ecommerceCreateCategoriesBatchCmd = &cobra.Command{
	Use:   "create-categories-batch",
	Short: "Create categories in batch",
	RunE:  runEcommerceCreateCategoriesBatch,
}

var ecommerceCreateCategoriesBatchFlags struct {
	categories    []string
	updateEnabled bool
	body          string
}
...
func runEcommerceCreateCategoriesBatch(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "create-categories-batch",
	...
	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/categories/batch", pathParams),
	...
	if ecommerceCreateCategoriesBatchFlags.body != "" {
		if err := json.Unmarshal([]byte(ecommerceCreateCategoriesBatchFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("categories") {
		bodyMap["categories"] = ecommerceCreateCategoriesBatchFlags.categories
	}
	if cmd.Flags().Changed("update-enabled") {
		bodyMap["updateEnabled"] = ecommerceCreateCategoriesBatchFlags.updateEnabled
	}
	req.Body = bodyMap
