...
var conversationsUpdatePushedMessageCmd = &cobra.Command{
	Use:   "update-pushed-message",
	Short: "Update an automated message",
	RunE:  runConversationsUpdatePushedMessage,
}

var conversationsUpdatePushedMessageFlags struct {
	id   string
	text string
	body string
}
...
func runConversationsUpdatePushedMessage(cmd *cobra.Command, args []string) error {
	...
	pathParams["id"] = fmt.Sprintf("%v", conversationsUpdatePushedMessageFlags.id)
	...
	if conversationsUpdatePushedMessageFlags.body != "" {
		if err := json.Unmarshal([]byte(conversationsUpdatePushedMessageFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("text") {
		bodyMap["text"] = conversationsUpdatePushedMessageFlags.text
	}
	req.Body = bodyMap
