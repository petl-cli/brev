...
var contactsUpdateMultipleCmd = &cobra.Command{
	Use:   "update-multiple",
	Short: "Update multiple contacts",
	RunE:  runContactsUpdateMultiple,
}

var contactsUpdateMultipleFlags struct {
	contacts []string
	body     string
}
...
func runContactsUpdateMultiple(cmd *cobra.Command, args []string) error {
	...
	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/contacts/batch", pathParams),
	...
	if contactsUpdateMultipleFlags.body != "" {
		if err := json.Unmarshal([]byte(contactsUpdateMultipleFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("contacts") {
		bodyMap["contacts"] = contactsUpdateMultipleFlags.contacts
	}
	req.Body = bodyMap
