...
var transactionalEmailsAddBlockedDomainCmd = &cobra.Command{
	Use:   "add-blocked-domain",
	Short: "Add a new domain to the list of blocked domains",
	RunE:  runTransactionalEmailsAddBlockedDomain,
}

var transactionalEmailsAddBlockedDomainFlags struct {
	domain string
	body   string
}
...
func runTransactionalEmailsAddBlockedDomain(cmd *cobra.Command, args []string) error {
	...
	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/smtp/blockedDomains", pathParams),
	...
	if transactionalEmailsAddBlockedDomainFlags.body != "" {
		if err := json.Unmarshal([]byte(transactionalEmailsAddBlockedDomainFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("domain") {
		bodyMap["domain"] = transactionalEmailsAddBlockedDomainFlags.domain
	}
	req.Body = bodyMap
