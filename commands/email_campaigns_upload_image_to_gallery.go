...
var emailCampaignsUploadImageToGalleryCmd = &cobra.Command{
	Use:   "upload-image-to-gallery",
	Short: "Upload an image to your account's image gallery",
	RunE:  runEmailCampaignsUploadImageToGallery,
}

var emailCampaignsUploadImageToGalleryFlags struct {
	imageUrl string
	name     string
	body     string
}
...
func runEmailCampaignsUploadImageToGallery(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "upload-image-to-gallery",
	...
	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/emailCampaigns/images", pathParams),
	...
	if emailCampaignsUploadImageToGalleryFlags.body != "" {
		if err := json.Unmarshal([]byte(emailCampaignsUploadImageToGalleryFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("image-url") {
		bodyMap["imageUrl"] = emailCampaignsUploadImageToGalleryFlags.imageUrl
	}
	if cmd.Flags().Changed("name") {
		bodyMap["name"] = emailCampaignsUploadImageToGalleryFlags.name
	}
	req.Body = bodyMap
