...
var masterAccountUpdateSubAccountPlanCmd = &cobra.Command{
	Use:   "update-sub-account-plan",
	Short: "Update sub-account plan",
	RunE:  runMasterAccountUpdateSubAccountPlan,
}

var masterAccountUpdateSubAccountPlanFlags struct {
	id   int
	body string
}
...
func runMasterAccountUpdateSubAccountPlan(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "update-sub-account-plan",
	...
	pathParams["id"] = fmt.Sprintf("%v", masterAccountUpdateSubAccountPlanFlags.id)

	req := &httpclient.Request{
		Method:      "PUT",
		Path:        httpclient.SubstitutePath("/corporate/subAccount/{id}/plan", pathParams),
	...
	if masterAccountUpdateSubAccountPlanFlags.body != "" {
		if err := json.Unmarshal([]byte(masterAccountUpdateSubAccountPlanFlags.body), &bodyMap); err != nil {
