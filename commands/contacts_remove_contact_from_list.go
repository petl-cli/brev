...
var contactsRemoveContactFromListCmd = &cobra.Command{
	Use:   "remove-contact-from-list",
	Short: "Delete a contact from a list",
	RunE:  runContactsRemoveContactFromList,
}

var contactsRemoveContactFromListFlags struct {
	listId int
	body   string
}
...
func runContactsRemoveContactFromList(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "remove-contact-from-list",
	...
	pathParams["listId"] = fmt.Sprintf("%v", contactsRemoveContactFromListFlags.listId)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/contacts/lists/{listId}/contacts/remove", pathParams),
	...
	if contactsRemoveContactFromListFlags.body != "" {
		if err := json.Unmarshal([]byte(contactsRemoveContactFromListFlags.body), &bodyMap); err != nil {
