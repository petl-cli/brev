...
var emailCampaignsSendReportCmd = &cobra.Command{
	Use:   "send-report",
	Short: "Send the report of a campaign",
	RunE:  runEmailCampaignsSendReport,
}
...
func runEmailCampaignsSendReport(cmd *cobra.Command, args []string) error {
	...
	pathParams["campaignId"] = fmt.Sprintf("%v", emailCampaignsSendReportFlags.campaignId)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/emailCampaigns/{campaignId}/sendReport", pathParams),
	...
	if emailCampaignsSendReportFlags.body != "" {
		if err := json.Unmarshal([]byte(emailCampaignsSendReportFlags.body), &bodyMap); err != nil {
